...
def entry_from_fvg_v10(ctx, su, fvg):
    """Canonical registered FVG-edge geometry for one already-selected FVG.

    Selection and ownership are intentionally outside this helper.  V1 calls
    it with its legacy freshest-FVG selection; A Continuation V2 calls it only
    after proving that the FVG belongs to the BOS impulse.
    """
    bull = su['dr'] == 'LONG'; bb = su['bos_bar']
    entry = fvg[1] if bull else fvg[0]
...
def find_entry_v10(ctx, su):
    ...
    return entry_from_fvg_v10(ctx, su, fvl[-1])
