...
<h2>Draw on Liquidity <span class="mut">· A/B diagnostics</span></h2>
<div class="sub">Read-only ranked DOL metadata. It does not filter signals or change execution.</div>
...
function render(d){var rows=d.rows||[], attached=rows.filter(function(r){return r.metadata_status==='ATTACHED';});
 ...
 document.getElementById('cards').innerHTML=card('Tagged signals',attached.length)+card('Open DOL',open)+card('Complete narrative',complete)+card('Direction aligned',aligned);
 var head='<tr><th>Signal</th><th>Dir</th><th>Model / catalyst</th><th>Selected DOL</th><th>Tier</th><th>Status</th><th>Source</th><th>Narrative</th><th>Aligned</th><th>Stacked constituents</th><th>Successor DOL</th></tr>';
 var body=rows.map(function(r){var cls=(r.dir||'').toLowerCase(), st=(r.dol_status||'').toLowerCase();return '<tr><td>'+esc(r.date+' '+r.bos)+'</td><td class="'+cls+'"><b>'+esc(r.dir)+'</b></td><td>'+esc(r.model)+'<br><span class="mut">'+esc(r.cat)+'</span></td><td><b>'+esc(r.dol_price)+'</b><br><span class="mut">'+esc(r.selected_dol)+'</span></td><td><span class="pill">T'+esc(r.dol_tier)+'</span></td><td class="'+st+'"><b>'+esc(r.dol_status)+'</b></td><td class="'+(r.source_present?'yes':'no')+'">'+(r.source_present?'YES':'NO')+'</td><td>'+esc(r.narrative_class)+'</td><td class="'+(r.direction_aligned_with_dol?'yes':'no')+'">'+(r.direction_aligned_with_dol?'YES':'NO')+'</td><td class="const">'+constituents(r.stacked_constituents)+'</td><td>'+pool(r.successor_dol)+'</td></tr>';}).join('');
 if(!rows.length)body='<tr><td colspan="11" class="empty">No DOL-tagged live A/B signals yet. New signals will appear here automatically.</td></tr>';
