...
<body><h2>DOL Delivery Reversal <span class="mut">· shadow</span></h2><div class="mut">Liquidity delivered → causal Reversal → OPEN DOL in the new direction → fixed 2R. <span class="warn">SHADOW ONLY · NO BROKER EXECUTION</span></div><div class="cards" id="stats"></div><div class="grid" id="current"></div><div class="wrap"><table id="rows"></table></div>
...
function render(d){let s=d.stats||{},r=d.current_signal||{},t=d.current_trade||r,l=d.last_result||{};stats.innerHTML=card('Status',d.status)+card('Trades',s.trades||0)+card('WR %',s.WR_pct==null?'—':f(s.WR_pct,2)+'%')+card('PF',s.PF==null?'—':f(s.PF,3))+card('Net R',f(s.Net_R||0,3))+card('Net $',f(s.Net_USD||0,0));current.innerHTML=cell('Candidate',r.candidate_id)+cell('Direction',r.direction)+cell('Catalyst',r.catalyst)+cell('Narrative',r.narrative_class)+cell('Source families',(r.source_families||[]).join(', '))+cell('Open DOL',r.selected_dol)+cell('DOL price',r.dol_price)+cell('DOL tier',r.dol_tier_class)+cell('Entry',r.theoretical_entry)+cell('SL',r.SL)+cell('Fixed 2R',r.TP)+cell('Take?',r.accepted_by_DOL_DELIVERY_REVERSAL===true?'YES':r.accepted_by_DOL_DELIVERY_REVERSAL===false?'NO':'—')+cell('State',t.current_trade_state)+cell('Last result',l.final_shadow_outcome?(l.final_shadow_outcome+' · '+e(l.realized_R)+'R'):'—');let h='<tr><th>Recorded</th><th>Dir</th><th>Catalyst</th><th>Narrative</th><th>DOL</th><th>DOL px</th><th>Tier</th><th>Take</th><th>Entry</th><th>SL</th><th>2R</th><th>State</th><th>Result</th><th>R</th></tr>';let b=(d.rows||[]).map(x=>'<tr><td>'+e(x.recorded_at)+'</td><td class="'+e((x.direction||'').toLowerCase())+'">'+e(x.direction)+'</td><td>'+e(x.catalyst)+'</td><td>'+e(x.narrative_class)+'</td><td>'+e(x.selected_dol)+'</td><td>'+e(x.dol_price)+'</td><td>'+e(x.dol_tier_class)+'</td><td class="'+(x.accepted_by_DOL_DELIVERY_REVERSAL?'yes':'no')+'">'+(x.accepted_by_DOL_DELIVERY_REVERSAL?'YES':'NO')+'</td><td>'+e(x.theoretical_entry)+'</td><td>'+e(x.SL)+'</td><td>'+e(x.TP)+'</td><td>'+e(x.current_trade_state)+'</td><td>'+e(x.final_shadow_outcome)+'</td><td>'+e(x.realized_R)+'</td></tr>').join('');rows.innerHTML=h+(b||'<tr><td colspan="14" class="mut">No forward observations yet.</td></tr>')}
