...
## v32.3 - A/B-shallow independent risk budget (2026-07-31)

- Deep `A/B` keeps the normal `RISK_PCT` budget.
- `A/B-shallow` is sized independently using `AB_SHALLOW_RISK_PCT` (default: the same as `RISK_PCT`).
- Default production request: 0.5% deep + 0.5% shallow, so maximum combined setup exposure is 1.0% when both orders fill.
- Removed `AB_SHALLOW_RISK_SHARE`; sibling quantities are no longer split from one shared budget.
- The table/guard setup grouping is retained so both sibling rows can coexist and are displayed next to each other.
- `live_emit.size_for` now accepts a per-signal risk percentage so alerts and execution use the same shallow risk.
- Previous v32.2 50/50 backtest results do not apply to this doubled-risk configuration.
...
## v32.2 - causal A/B-shallow sibling, split risk and fixed 2R (2026-07-31)

This release adds an optional execution sibling beside the existing A/B row. It does not change the A/B detector.

### Important correction

- The earlier retrospective 399-case analysis was not deployable because it selected trades after observing that the original target was reached before the original entry.
- v32.2 removes that look-ahead. `A/B-shallow` is built at signal-bar close for every eligible A/B signal using only information already known at that time.

### Production design

- Current A/B remains the deep entry.
- `A/B-shallow` is placed at 25% of the distance from the signal close to the deep entry.
- Both rows use the detector structural stop. The shallow stop is therefore normally wider, not tighter.
- The shallow target is fixed 2R by default; 3R is available only as a configurable challenger.
- Total account risk is split. Recommended deployment: 50% deep A/B and 50% A/B-shallow; risk is not doubled.
- Both siblings share one `setup_group_id`, so max-trades/day, loss count, P&L and the one-position slot treat them as one setup.
- The table shows adjacent `A/B` and `A/B-shallow` rows with separate entries, quantities, SL, TP and outcomes.
- Sibling sending is fail-closed: a partial batch triggers cancellation instead of leaving an unintended one-sided split.

### Causal replay result

On 1,029 chronological A/B signals, modeled with the project fill rules, costs and approximate guardrails:

- Current A/B: $108,949.82, max DD $3,032.96, 65.12% group win rate.
- Recommended 50/50 split with shallow 2R: $117,986.74, max DD $2,152.44, 60.71% group win rate.
- Difference: +$9,036.92 (+8.29%) and 29.0% lower drawdown, but a lower win rate.
- The 3R split produced less long-run net profit and a lower shallow-leg win rate, so 2R is the production candidate.

Deployment: `V32_2_AB_SHALLOW_DEPLOY_PL.md`.
