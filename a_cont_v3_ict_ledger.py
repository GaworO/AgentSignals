    ...
    with _connect() as con:
        ...
        for trigger in triggers:
            ...
            con.execute(
                ...
                (cid, "CLOSE_THROUGH", tms, _iso(trigger.get("trading_day")), stage, status,
...
async function dash(){let s=await j('/continuation/api/status');$('#app').innerHTML=`<div class="top"><div><h2>MNQ Continuation · Shadow dashboard</h2><div class="mut">Policy B unmanaged · frozen OPEN bullish DOL · no broker actions</div></div><div class="badge">SHADOW ONLY · ${esc(s.engine.status)}</div></div>`+cards(s)+`<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(300px,1fr))"><div class="card metric"><h3>Open positions</h3><pre>${esc(JSON.stringify(s.open_positions,null,2))}</pre></div><div class="card metric"><h3>Historical Development reference only</h3><pre>${esc(JSON.stringify(s.historical_development_reference_only,null,2))}</pre></div><div class="card metric"><h3>Execution contract</h3><p>Final target is the causal OPEN bullish DOL frozen at decision. Structural SL, one-tick-through fill, 10-minute expiry, no manager, fill-bar brackets disabled, then adverse-first.</p></div></div>`}
async function list(){let q=new URLSearchParams(location.search),d=await j('/continuation/api/candidates?'+q),rows=d.rows||[],s=d.funnel_stats||{},reasons=[...new Set(rows.map(x=>x.rejection_reason).filter(Boolean))].sort();let body=rows.map(x=>{let p=x.payload||{},canonical=x.event_kind==='CANONICAL_OUTPUT',thesis=p.jade_thesis==='LONG',close=x.event_kind==='CLOSE_THROUGH'||p.source_event?.close_through===true,confirmed=canonical,dol=x.eligible===1&&!!x.dol_id,risk=dol&&x.entry_price!=null&&x.stop_price!=null,filled=x.order_state==='FILLED',ready=x.order_state==='PENDING',tracking=x.status==='TRACKING',status=filled?'FILLED':x.trade_state==='CLOSED'?'CLOSED':ready?'READY / ORDER PENDING':tracking?'TRACKING':x.status==='SUPERSEDED'?'CANONICAL CONFIRMED':'REJECTED',cls=(filled||ready||x.trade_state==='CLOSED')?'green':tracking||x.status==='SUPERSEDED'?'warn':'red',finalState=(filled||ready||x.trade_state==='CLOSED')?'ok':tracking?'wait':'bad';return `<div class="card"><div class="head"><div class="title"><a href="/continuation/candidate/${encodeURIComponent(x.candidate_id)}"><span class="green">LONG</span> · ${esc(x.bsl_name||p.cat||'LIQUIDITY')} · ${esc(x.decision_at)}</a></div><div class="${cls}"><b>${esc(status)}</b></div></div><div class="steps">${step(1,'HTF LONG thesis',esc(p.jade_thesis||'not yet available')+' · fixed at 18:00 ET',thesis?'ok':'bad')}${step(2,'BSL close-through',esc(x.bsl_name||p.source_event?.bsl_name)+' @ '+esc(p.source_event?.bsl_price||p.bsl_price),close?'ok':'bad')}${step(3,'Displacement + owned FVG',canonical?('FVG '+esc(p.fvg_lo)+'–'+esc(p.fvg_hi)):'Waiting for canonical chain',confirmed?'ok':tracking?'wait':'bad')}${step(4,'Pullback + hold + BOS',canonical?('BOS '+esc(p.bos_iso||p.bos_ms)):'Not confirmed',confirmed?'ok':tracking?'wait':'bad')}${step(5,'OPEN bullish DOL',dol?(esc(x.dol_id)+' @ '+fmt(x.target_price,2)):esc(x.rejection_reason||'not selected'),dol?'ok':'bad')}${step(6,'Entry + risk / fill',risk?('Entry '+fmt(x.entry_price,2)+' · SL '+fmt(x.stop_price,2)+' · DOL TP '+fmt(x.target_price,2)):status,finalState)}</div><div class="levels"><b>Candidate:</b> ${esc(x.candidate_id)} <b>Stage:</b> ${esc(x.stage)} <b>Order:</b> ${esc(x.order_state)} <b>Trade:</b> ${esc(x.trade_state)} ${x.net_r==null?'':('<b>Net:</b> '+fmt(x.net_r)+'R')}</div><div class="reason"><b>Decision reason:</b> ${esc(x.rejection_reason||status)}</div></div>`}).join('')||'<div class="empty">No Continuation candidates recorded yet. Scanner is waiting for the first causal close-through event.</div>';$('#app').innerHTML=`<div class="top"><div><h2>MNQ Continuation · Candidates</h2><div class="mut">Live funnel for every independent Continuation event. Reversal emissions and broker guards do not decide whether this setup exists.</div></div><div class="badge">SHADOW ONLY · independent scanner</div></div><div class="kpis">${k('Candidates',s.candidates||0)}${k('Close-through',s.close_through||0)}${k('Canonical confirmed',s.canonical_confirmed||0)}${k('HTF LONG',s.htf_long||0)}${k('DOL aligned',s.dol_aligned||0)}${k('Filled',s.filled||0)}</div><div class="help"><b>What each step means</b><br><span class="mut">1 JadeCap HTF LONG thesis fixed at 18:00 ET → 2 registered BSL close-through → 3 displacement + event-owned FVG → 4 later pullback/hold + BOS → 5 causal OPEN bullish DOL → 6 frozen Entry/SL/DOL target and shadow fill. Incomplete and rejected physical events remain visible.</span></div><div class="filters"><select id="status"><option value="">all status</option>${['TRACKING','REJECTED','SUPERSEDED','UNFILLED_OR_PENDING'].map(x=>`<option>${x}</option>`)}</select><select id="reason"><option value="">all reasons</option>${reasons.map(x=>`<option>${esc(x)}</option>`)}</select><input id="start" type="date"><input id="end" type="date"><button id="go">Filter</button></div><div class="cards">${body}</div>`;for(let z of ['status','reason','start','end'])$('#'+z).value=q.get(z)||'';$('#go').onclick=()=>{let z=new URLSearchParams();for(let a of ['status','reason','start','end'])if($('#'+a).value)z.set(a,$('#'+a).value);location.search=z}}
