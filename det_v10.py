...
def _trc(trigger,dr,model,name,stage,disp=None,details=None):
    if not _DBG: return
...
def emit(t,model,name,dr,disp,conf=None):   # wejscie v10 (FVG-edge/OTE), SL=CE disp, TP 2R, BEZ filtrow dopasowania
    ...
    record=dict(brk=_cur_break,date=str(dates[su['bos_bar']]),model=model,cat=name,dir=dr,
    ...
    _trc(t,dr,model,name,'POTWIERDZONY',disp,details={
        'date':record['date'],'bos':record['bos'],'bos_ms':record['bos_ms'],
        'entry':record['entry'],'SL':record['SL'],'TP':record['TP'],'risk':record['risk'],
        'signal_close':round(float(cl[su['bos_bar']]),10)})
    out.append(record)
