...
import ab_shallow        # causal sibling entry; independent risk, fixed 2R/3R target
...
VERSION = 'v32.3-ab-shallow-independent-risk2R'   # A/B and shallow each have their own risk budget
...
def _exec_order(x, text=None):
    ...
    try:
        ...
        if _q.isdigit() and int(_q) > 0:
            ...
            try:
                _sf = live_emit.size_for(e, sl, x.get('_risk_pct_override')); qty = int(_sf[0]) if _sf else 1
            except Exception:
                qty = 1
        ...
        qty = int(qty)
        if qty < 1 and x.get('_allow_zero_qty'):
            return {"sent": False, "reason": "qty_zero", "status": 0, "qty": 0}
        qty = max(1, qty)
...
def _prepare_ab_siblings(repx):
    """Build the production A/B + A/B-shallow sibling set.

    The detector signal is unchanged. Deep A/B uses ``RISK_PCT`` and the
    shallow sibling uses its own ``AB_SHALLOW_RISK_PCT``. If both fill, the
    combined maximum setup exposure is the sum of those two percentages.
    """
    repx['_strat'] = repx.get('_strat', 'A/B')
    if repx['_strat'] != 'A/B' or not ab_shallow.enabled():
        return [repx]
    # The first live routing trades stay single-contract deep A/B.  Splitting a
    # one-contract ramp into two forced one-contract orders would double risk.
    ...
    try:
        ...
        risk_meta = ab_shallow.independent_risk_metadata(repx, shallow)
    except Exception as e:
        repx['_shallow_skip'] = str(e)
        print('A/B-shallow build skip:', e, flush=True)
        return [repx]
    repx['_ab_independent_risk'] = risk_meta
    shallow['_ab_independent_risk'] = risk_meta
    # Do not set a quantity override. _exec_order sizes each sibling independently
    # through the same normal auto-sizing path.
    return [repx, shallow]

def _exec_sibling_batch(items, base_text):
    """Send all sibling orders fail-closed; cancel everything on a partial batch."""
    results = []
    for item in items:
...
def _signal_bar_close(x):
    """Read the detector signal-bar close from the same rolling buffer.

    A/B-shallow must be causal.  It therefore uses the close of ``bos_bar`` and
    never the current/latest price seen after the signal.
    """
    try:
        wanted = int(x.get('bos_bar'))
        with open(BUF, newline='', encoding='utf-8') as f:
            for i, row in enumerate(csv.DictReader(f)):
                if i == wanted:
                    return float(row['close'])
    except Exception as e:
        print('signal close lookup err', e, flush=True)
    return None
...
def _process_new(now_ms=None):
    ...
    for tk, members in groups.items():
        ...
        if os.environ.get('EXEC_WEBHOOK') or os.environ.get('EXEC_FX') == '1':   # FX services: MetaApi, no webhook
            ...
            if _gmode == 'off':
                ...
                if _gok:
                    ...
                    _okbatch, _batch = _exec_sibling_batch(_book_items, txt)
                    if _okbatch:
                        for _item, _res, _itxt in _batch: guardrails.note(_item, 'manual')
                        code = 'exec-manual'
                    else:
                        for _item, _res, _itxt in _batch: guardrails.note(_item, 'blocked', 'batch_exec_failed')
                        _exec_fail_alert({'batch':[r for _,r,_ in _batch]}, 'manual'); code = 'exec-failed'
                else:
                    code = _blk(_gwhy)
                ...
                if _gok:
                    ...
                    if _okbatch:
                        ...
                        for _item, _res, _itxt in _batch: guardrails.note(_item, 'blocked', 'batch_exec_failed')
                        _exec_fail_alert({'batch':[r for _,r,_ in _batch]}, 'auto'); code = 'exec-failed'
                else:
                    code = _blk(_gwhy)
        else:
        ...
        for _item in _book_items:
            try: _save_db(_item, (_item.get('_alert_txt') or txt), code)
            except Exception as e: print('save_db item err', e, flush=True)
        ...
        try:
            if code in ('exec', 'exec-manual'):
                for _item in _book_items:
                    if _item.get('_execution_plan'): manage.register(_item, TRADES)
        except Exception as e: print('manage.register err', e, flush=True)
        try:
            for _item in _book_items:
                if not _item.get('_execution_plan'):
                    # Blocked candidates still receive normalised timing/price semantics.
                    _candidate_plan = execution_plan.build_execution_plan(
                        _item, 1, strategy=_item.get('_strat', 'A/B'), signal_key=guardrails._skey(_item))
                    execution_plan.attach_plan(_item, _candidate_plan)
                shadow.record_plan(_item['_execution_plan'])
...
def status():
    ...
    _body=dict(version=VERSION, strategy_clock='UTC-04:00 fixed', utc_ms=timebase.now_ms(), primed=_primed, archive_bars=na, **_last,
               ...
               ab_shallow_combined_max_risk_pct=(float(os.environ.get('RISK_PCT','0.5') or 0.5) + float(os.environ.get('AB_SHALLOW_RISK_PCT', os.environ.get('RISK_PCT','0.5')) or 0.5)),
